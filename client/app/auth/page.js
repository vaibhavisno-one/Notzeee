"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { Loader2, AlertCircle } from "lucide-react";
import Link from "next/link";

export default function AuthPage() {
    const [status, setStatus] = useState("idle"); // idle, loading, error, success
    const router = useRouter();

    const handleSignIn = async () => {
        setStatus("loading");
        // Mock network request
        setTimeout(() => {
            // Simulate success 90% of time for demo, or just success.
            // User requirement: "Success redirect to /app"
            setStatus("success");
            router.push("/app");
        }, 1500);
    };

    return (
        <div className="flex min-h-screen items-center justify-center bg-neutral-50 px-6">
            <div className="w-full max-w-sm bg-white border border-neutral-200 rounded-lg p-8 shadow-sm">
                <div className="mb-8">
                    <Link href="/" className="inline-block text-xl font-bold font-editorial text-neutral-900 mb-2">Notzeee</Link>
                    <h2 className="text-lg font-medium text-neutral-700">Welcome back</h2>
                </div>

                {status === "error" && (
                    <div className="mb-4 p-3 bg-red-50 border border-red-100 rounded text-sm text-red-600 flex items-center gap-2">
                        <AlertCircle size={16} />
                        <span>Unable to connect. Please try again.</span>
                    </div>
                )}

                <button
                    onClick={handleSignIn}
                    disabled={status === "loading" || status === "success"}
                    className="w-full bg-white border border-neutral-200 text-neutral-900 font-medium py-2.5 px-4 rounded-md hover:bg-neutral-50 transition-colors flex items-center justify-center gap-3 shadow-sm disabled:opacity-70 disabled:cursor-not-allowed"
                >
                    {status === "loading" ? (
                        <>
                            <Loader2 size={18} className="animate-spin text-neutral-400" />
                            <span className="text-neutral-500">Connecting...</span>
                        </>
                    ) : (
                        <>
                            {/* Google Icon SVG */}
                            <svg className="w-5 h-5" viewBox="0 0 24 24">
                                <path
                                    d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                                    fill="#4285F4"
                                />
                                <path
                                    d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                                    fill="#34A853"
                                />
                                <path
                                    d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                                    fill="#FBBC05"
                                />
                                <path
                                    d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                                    fill="#EA4335"
                                />
                            </svg>
                            <span>Continue with Google</span>
                        </>
                    )}
                </button>

                <p className="mt-8 text-center text-xs text-neutral-400">
                    By clicking continue, you agree to our <span className="underline cursor-pointer hover:text-neutral-600">Terms</span> and <span className="underline cursor-pointer hover:text-neutral-600">Privacy Policy</span>.
                </p>

            </div>
        </div>
    );
}
